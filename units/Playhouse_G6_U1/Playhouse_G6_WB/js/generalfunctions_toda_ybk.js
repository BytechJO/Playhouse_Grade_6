...
function getCurrFileOrDirectory(aNeed, aPath) {
    ...
    return (aNeed == 'file') ? lastPathSegment : currFolder;
}
...
$(window).on("orientationchange", function (event) {
    ...
    if ($('.activity_area').data('type') == 'linedraw' || $('.activity_area').data('type') == 'dragndrop') {
        $('.loading').show();
...
function alignStage() {
    ...
    //    console.log('alignStage >> ', screenContentHeight, $('.all_cont').height(), 'allcont>>>', $('.activity_area').data('type'));
    // console.log('alignStage >> ', theActivity.orientationAdjust);
