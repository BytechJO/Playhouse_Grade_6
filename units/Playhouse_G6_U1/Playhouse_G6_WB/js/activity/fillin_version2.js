...
function normalizeAnswer(str) {
  return (
    $.trim(String(str))
      .toLowerCase()
      // نخلي الأرقام و ":" عشان 8:30
      // ونشيل علامات الترقيم اللي ما بدنا ندقق عليها
      .replace(/[.,!?;'"’”“()\[\]{}]+/g, "")
      // نوحّد المسافات
      .replace(/\s+/g, " ")
  );
...
FillIn.prototype = {
  ...
  validate: function () {
    ...
    $area.find(".que").each(function () {
      ...
      if (_case == "no") {
        _cAns = _cAns.map(function (ans) {
          return normalizeAnswer(ans);
        });
      }
                  ...
                  if (_cAlternateAns != undefined) {
                    ...
                    if (fDataObj.alternateanswer[thisInd] != undefined) {
                      if (fDataObj.alternateanswer[thisInd].length > 0) {
                        _cAltAns = getStrArray(
                          fDataObj.alternateanswer[thisInd],
                          "activity",
                        );
                        if (_case == "no") {
                          _cAltAns = _cAltAns.map(function (ans) {
                            return normalizeAnswer(ans);
                          });
                        }
                      }
