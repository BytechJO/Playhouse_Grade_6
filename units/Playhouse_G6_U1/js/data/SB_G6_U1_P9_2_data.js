var stereo_data = {
  audio: "",
  exist: false,
  bgColor_rgb: "rgb(0, 183, 206)",
  type: "text",
  playListData: [
    {
      audiourl: "../audios/demo.mp3",
    },
    {
      url: "",
    },
    {
      url: "",
    },
    {
      url: "",
    },
  ],
};
var mcq_data = {
  layout: 1,
  numinrow: [[1]],
  mainTitle: "../images/pages/sb-icons/language-building.png",
  mainTitleIcon: "../images/pages/sb-icons/conv_main_title_icon.png",
  mainTitleIconPos: { right: "80px" },
  mainTitleAudio: "../audios/P9/LANGUAGE_BUILDING.mp3",
  subTitleTextLeft: "<span class='title-order'>2</span> <b>Circle it!</b>",
  subTitleTextRight:
    "Look at the pictures and decide which sentence is better. Circle it.",
  subTitleIcons: [""],
  subTitleAudio: "../audios/new/circle.mp3",
  activityheading: "",
  activitysubheading: "",
  activityicon: "../images/icons/hand_icon.png",
  select: "single", // single (or) multiple
  shape: "roundrect", // circle (or) roundrect (or) rectangle (or) svg (or) cross (or) checkbox
  bgcolor: "none",
  numbering: "alphabet",
  numberstartfrom: "a",
  numberofcolumns: 1,
  questions: [
    {
      question: "",
      image: "../images/pages/activities/p9_2_1.png",
      answer: [2],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "I like to eat the apple for breakfast.",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "I like to eat an apple for breakfast.",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/p9_2_2.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "The red house is smaller than the others",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "A red house is smaller than the others.",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "../images/pages/activities/p9_2_3.png",
      answer: [1],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "He gave him the money.",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "He gave him a money.",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
  ],
};
