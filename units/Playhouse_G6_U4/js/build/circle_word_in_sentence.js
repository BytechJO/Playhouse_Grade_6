	...
	if(typeof aObj !=undefined && aObj !=null){		
			...
			for(y= 0;y<numOfQinCol;y++){
				...
				if(typeof tpOb != undefined && tpOb != null ){
                    ...
                    for (var rr = 0; rr < (opts).length; rr++) {
                        fillBoxesArr[rr]='';                       
						if(tpOb.inputbox == "no"){
							tmphtml = "<span class='pick_set d-flex flex-column' id='pick_set_"+currQueNum+"_"+(rr+1)+"'>";
						}else{
							tmphtml = "<span class='pick_set d-flex flex-column' id='pick_set_"+currQueNum+"_"+(rr+1)+"'><input class='text_input_area' type='text' readonly disabled>";
						}						

						tmphtml += '<div class="pick_box">';
                        var tmpOpt = opts[rr];
		...
		if(aObj.image != undefined && aObj.image != ''){
            // if(aObj.imageposition == 'back'){
                htmlStmt += '<div class="img_container"><img src="'+aObj.image+'"></img></div>';
            // }
        }		
