...
function buildHeader(aObj) {
    ...
    var homeLink =
        (_templatePath == "." ? "../" : "") +
        "../../../" +
        aObj.homebutton.link;

    var fStmt = "";

    if (typeof aObj != undefined && aObj != null) {
        if (
            typeof aObj.bgcolor != undefined &&
            aObj.bgcolor != null
        ) {
        ...
        fStmt +=
            "<div class='d-flex justify-content-between header_wrap'>";
        ...
        if (
            typeof aObj.title.text != undefined &&
            aObj.title.text != null
        ) {
            fStmt +=
        ...
        if (isMobile()) {
            fStmt +=
                "<div class='full_screen hideInSmall my-auto'>";
...
function buildFooter(aObj) {
    ...
    if (
        ...
        /* Student Pages menu button */
        fStmt +=
            '<div class="studentPagesToggle hamburgerBtn" id="studentPagesToggle">';
        ...
        if (
            ...
            aObj.buttons.length ==
                aObj.filetoload.length
        ) {
            for (
                var f = 0;
                f < aObj.buttons.length;
                f++
            ) {
                fStmt +=
...
function buildSubFooter(aObj, aVal) {
    var thisFile = getCurrFileOrDirectory(
        "file"
    ).toLowerCase();

    var thisFolder = getCurrFileOrDirectory(
        "directory"
    ).toLowerCase();
    ...
    aVal =
        typeof aVal != undefined && aVal != null
            ? aVal
            : "no";
    ...
    if (
        ...
        if (
            ...
            fStmt +=
                '<div class="d-flex func_ctrls">';

            for (
                var ff = 0;
                ff <
                aObj.functionbuttons.length;
                ff++
            ) {
                tsubBtn =
            ...
            $(".sub_footer_buttons_wrap").append(
                fStmt
            );
...
function buildStudentPagesSidebar() {
    if (
        ...
        console.warn(
            "_activityData.list is not available"
        );

        return;
    }

    $("#studentPagesSidebar").remove();
    $("#studentPagesSidebarOverlay").remove();
    ...
    var sStmt =
        '<div class="studentPagesSidebar" id="studentPagesSidebar">';

    sStmt +=
        '<div class="studentPagesSidebarHeader">';

    sStmt += "<span>Student Pages</span>";

    sStmt +=
        '<span class="studentPagesSidebarClose" id="studentPagesSidebarClose">&times;</span>';

    sStmt += "</div>";

    sStmt +=
        '<div class="studentPagesSidebarList">';
    ...
    $.each(
        ...
        function (index, page) {
            if (
                ...
                sStmt +=
                    '<a class="studentPagesSidebarItem ' +
                    activeClass +
                    '" href="' +
                    page.file +
                    '">';
    ...
    sStmt +=
        '<div class="studentPagesSidebarOverlay" id="studentPagesSidebarOverlay"></div>';

    $("body").append(sStmt);

    $(document)
        .off(
            "click.studentPagesMenu",
            "#studentPagesToggle"
        )
        .on(
            "click.studentPagesMenu",
            ...
            function (event) {
                ...
                $("#studentPagesSidebar").addClass(
                    "open"
                );

                $("#studentPagesSidebarOverlay").addClass(
                    "show"
                );
            }
        );
    ...
    $(document)
        .off(
            "click.studentPagesMenuClose",
            "#studentPagesSidebarClose, #studentPagesSidebarOverlay"
        )
        .on(
            "click.studentPagesMenuClose",
            "#studentPagesSidebarClose, #studentPagesSidebarOverlay",
            function () {
                $("#studentPagesSidebar").removeClass(
                    "open"
                );

                $("#studentPagesSidebarOverlay").removeClass(
                    "show"
                );
            }
        );
    ...
    $(document)
        .off("keydown.studentPagesMenu")
        .on(
            "keydown.studentPagesMenu",
            function (event) {
                if (event.key === "Escape") {
                    $("#studentPagesSidebar").removeClass(
                        "open"
