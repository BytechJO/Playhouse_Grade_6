    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      if (_uAns.length > 0 && _cAns.length == _uAns.length) {
        for (var cc = 0; cc < _cAns.length; cc++) {
          var userAnswer = normalizeAnswer(_uAns[cc]);

          var correctAnswer = normalizeAnswer(_cAns[cc]);
          ...
          if (!isCorrect) {
            ...
            if (fDataObj.alternateanswer && fDataObj.alternateanswer[cc]) {
              alternateAnswers = fDataObj.alternateanswer[cc];
            }

            for (var alt = 0; alt < alternateAnswers.length; alt++) {
              var normalizedAlternate = normalizeAnswer(alternateAnswers[alt]);

              if (userAnswer === normalizedAlternate) {
                isCorrect = true;
                break;
              }
            }
