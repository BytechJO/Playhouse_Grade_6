var stereo_data = {
  audio: "",
  exist: true,
  bgColor_rgb: "rgb(0, 183, 206)",
  type: "text",
  playListData: [
    {
      audiourl: "../audios/P77/2.mp3",
    },
    {
      url: "",
    },
    {
      url: "",
    },
    {
      url: "",
    },
  ],
};
var mcq_data = {
  layout: 1,
  numinrow: [[1]],
  mainTitle: "../images/pages/sb-icons/conv_main_title.png",
  mainTitleIcon: "../images/pages/sb-icons/conv_main_title_icon.png",
  mainTitleAudio: "../audios/P77/CONVERSATION.mp3",
  mainTitleIconPos: { right: "80px" },
  subTitleTextLeft: '<span class="title-order">2</span> Listen to it!',
  subTitleTextRight:
    "Listen to the students describe the kind of weather they like. Circle the Word Power words you hear.",
  subTitleIcons: [],
  subTitleAudio: "../audios/P77/2_Listen_to_it!.mp3",
  select: "multiple", // single (or) multiple
  shape: "roundrect", // circle (or) roundrect (or) rectangle (or) svg (or) cross (or) checkbox
  bgcolor: "none",
  images: [
    "../images/pages/activities/1-img-2.png",
    "../images/pages/activities/1-img-3.png",
  ],
  numbering: "number",
  numberstartfrom: 1,
  numberofcolumns: 1,
  questions: [
    {
      question: "",
      image: "no",
      answer: [1, 3, 5],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "•  meteorologist",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  forcast",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  storm",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  clear",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  temperature",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
    {
      question: "",
      image: "no",
      answer: [2, 3],
      audio: "no",
      audioenable: "default", // correct (or) default
      options: [
        {
          text: "•  hurricane",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  lightning",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  thunder",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  heatwave",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
        {
          text: "•  thermometer",
          image: "",
          audio: "no",
          audioenable: "default", // correct (or) default
        },
      ],
    },
  ],
};
