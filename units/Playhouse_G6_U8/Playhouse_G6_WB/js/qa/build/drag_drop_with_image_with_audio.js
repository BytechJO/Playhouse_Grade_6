	...
	for(let x=0; x<activity.numOfRow; x++){
		...
		for(let y=0; y<activity.numOfCol; y++){
			...
			// 	if(typeof(values)=="string"){
					...
					html += `<div class='flex-cell ' style="flex: 1;border: none; text-align: center;min-width: 25%;">`+ activity.questions[fieldIndex].replace(/___/g, ` <input type="text" class="droppable_div ${styleInputOption}" />`) +'</div>';
