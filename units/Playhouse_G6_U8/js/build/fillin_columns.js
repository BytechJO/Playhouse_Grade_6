function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    htmlStmt += "</div>";
    htmlStmt += "</div>";
    if (aObj.helpPopup != undefined) {
      htmlStmt += '<button type="button" class="exercise_help_btn">';
      htmlStmt += "Exercise 2";
      htmlStmt += "</button>";

      htmlStmt += '<div class="exercise_popup_overlay">';

      htmlStmt += '<div class="exercise_popup">';

      htmlStmt +=
        '<button type="button" class="exercise_popup_close">×</button>';

      htmlStmt += '<div class="exercise_popup_title">';
      htmlStmt += aObj.helpPopup.title;
      htmlStmt += "</div>";

      htmlStmt += '<div class="exercise_popup_content">';

      htmlStmt += '<div class="exercise_popup_left">';

      for (var hp = 0; hp < aObj.helpPopup.sentences.length; hp++) {
        htmlStmt += '<div class="exercise_popup_sentence">';
        htmlStmt += aObj.helpPopup.sentences[hp];
        htmlStmt += "</div>";
      }
      ...
      if (aObj.helpPopup.image != undefined && aObj.helpPopup.image != "") {
        htmlStmt += '<div class="exercise_popup_image">';
  ...
  $(".exercise_help_btn")
    .off("click")
    .on("click", function () {
      $(".exercise_popup_overlay").addClass("show");
    });

  $(".exercise_popup_close")
    .off("click")
    .on("click", function () {
      $(".exercise_popup_overlay").removeClass("show");
    });

  $(".exercise_popup_overlay")
    .off("click")
    .on("click", function (e) {
      if ($(e.target).hasClass("exercise_popup_overlay")) {
        $(".exercise_popup_overlay").removeClass("show");
      }
    });
