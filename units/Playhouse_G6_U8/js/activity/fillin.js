    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      if (_uAns.length > 0 && _cAns.length == _uAns.length) {
        ...
        for (var cc = 0; cc < _cAns.length; cc++) {
          var correctValue =
            _case == "yes" ? _cAns[cc] : _cAns[cc].toLowerCase();

          correctValue = correctValue.replace(/[^a-zA-Z0-9]/g, "");

          normalizedCorrect.push(correctValue);
        }

        // حتى ما نستخدم نفس الجواب الصحيح أكثر من مرة
        var usedAnswers = [];

        for (var uu = 0; uu < _uAns.length; uu++) {
          var userValue = _case == "yes" ? _uAns[uu] : _uAns[uu].toLowerCase();

          userValue = userValue.replace(/[^a-zA-Z0-9]/g, "");

          var foundMatch = false;

          for (var ca = 0; ca < normalizedCorrect.length; ca++) {
            // الجواب مطابق ولسا ما استخدمناه
            if (
              userValue == normalizedCorrect[ca] &&
              usedAnswers.indexOf(ca) == -1
            ) {
              usedAnswers.push(ca);
