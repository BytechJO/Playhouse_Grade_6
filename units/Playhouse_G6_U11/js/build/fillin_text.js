function buildFillInBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
		...
		htmlStmt += '<div class="screen_elements d-flex align-items-center h-100">';
		// options
		...
		if(aObj.images != '' && aObj.images != undefined){
			for(let i=0; i < aObj.images.length; i++){
				htmlStmt += '<img src="'+aObj.images[i]+'" class="q_img q_img_'+i+'">';
			}
		}
		...
		htmlStmt += '<div class="ques">'
			...
			for (x = 0; x < numOfQuestions; x++) {
				...
				var que;
				if(tmpObj.answer.length<1){
					que = "";
				}else{
					que = "que";
				}
				htmlStmt += '<div class="'+que+' background_audio img_fillin_gr d-flex flex-column" data-qno="' + (x + 1) + '">';
				...
				if(tmpObj.textFront != undefined && tmpObj.textFront != ""){
					...
					if(tmpObj.textFront != undefined && tmpObj.textFront){
						htmlStmt += '<span class="textFront">'+tmpObj.textFront+'</span>';
					}
				}
				...
				htmlStmt += '<div class="q_space d-flex">';
					if(tmpObj.textFront == "" || tmpObj.textFront == undefined){
						if(aObj.numbering != 'none'){
							htmlStmt += '<div class="q_num_space">';
							if(aObj.numbering == 'alphabet'){
								xx =(x == 1) ?aObj.numberstartfrom:nextChar(xx);					
					...
					if(tmpObj.textEnd != undefined && tmpObj.textEnd != ""){
						htmlStmt += '<span class="textEnd">'+tmpObj.textEnd+'</span>';
					}
				
				htmlStmt += '</div>';// - end  - fillin_gr / img_fillin_gr
			}
			htmlStmt += '</div>';
			if(aObj.image != 'no' && aObj.image != ""){
