 ...
 *
 * @copyright 2016-2017 timescraper.com
 * @version 0.5.1
...
if (!Array.prototype.findIndex) {
  ...
  function randomH() {
      "use strict";
      // console.log("hsv-rgb", rgb(360 * Math.random(), 100, 100));
      // console.log(rgb.apply(null, [36, 76, 86]));
      var randomColor = Colorpalette.prototype.HSVtoRGB(360 * Math.random(), 90, 100);
      var color = {
          r: parseInt(randomColor.r, 10),
          g: parseInt(randomColor.g, 10),
          b: parseInt(randomColor.b, 10),
          a: 1
      };
      return color;
  }
  ...
  function randomColor() {
      ...
      //     g: Math.floor(Math.random() * 0),//debug toxdo
      //     b: Math.floor(Math.random() * 0),//debug toxdo
      //     a: 1
      ...
      grindBulkFrame: function grindBulkFrame(sketchpad, bulkFrame, receivingHistory) {
          ...
          if (bulkFrame.evs && bulkFrame.evs.length) {
              while (bulkFrame.evs.length) {
                  if (!bulkFrame.evs[0].pts) {
                      console.debug('empty first frame `bulkFrame.evs[0].pts`');
                  }
                  firstTimestamp = bulkFrame.evs[0].pts && bulkFrame.evs[0].pts.t[0];
                  ...
                  for (i = bulkFrame.evs.length - 1; i >= 0; i -= 1) {
                      if (bulkFrame.evs[i].pts && bulkFrame.evs[i].pts.t.length > 1) {
                          honeycomb = scrape(bulkFrame.evs[i], lastTimestamp);
                          groundFrame.evs.push(honeycomb);
                      }
                      if (!bulkFrame.evs[i].pts || (bulkFrame.evs[i].pts && bulkFrame.evs[i].pts.t.length < 2)) {
  ...
  Room.prototype = {
      ...
      setViewport: function setViewport(viewportEl, user, sketchpad) {
          ...
          if (user.user_color && user.user_color.r !== undefined) {
              user.user_color_str = "rgb(" + user.user_color.r + "," + user.user_color.g + "," + user.user_color.b + ")";
          }
          if (user.user_color_str) {
              user.pointerEl.style.backgroundColor = user.user_color_str;
              user.usernameEl.style.backgroundColor = user.user_color_str;
              viewportEl.style.borderColor = user.user_color_str;
          }
  ...
  function Input(config) {
      ...
      this.listY = [];
      this.listP = [];
      this.prePoints = config.prePoints || [];
      this.lastT = NaN;
      this.lastX = NaN;
      this.lastY = NaN;
      this.lastP = NaN;
  ...
  Input.prototype = {
      ...
      getPoints: function getPoints() {
          ...
          var points = [];
          var that = this;
          this.listT.forEach(function(val, i) {
              var point = {
                  t: val,
                  x: that.listX[i],
                  y: that.listY[i],
      ...
      setPoints: function setPoints(points) {
          ...
          points.forEach(function (point) {
              that.listT.push(point.t);
              that.listX.push(point.x);
              that.listY.push(point.y);
          })
          console.info('setPoints', points);
      }
  ...
  // @license magnet:?xt=urn:btih:0b31508aeb0634b347b8270c7bee4d411b5d4109&dn=agpl-3.0.txt AGPL-3 https://www.gnu.org/licenses/agpl-3.0-standalone.html
  /**
   * @source: http://developers.sketchpad.pro/dist/src/sketchpad.tool.aipen.js
   *
   * Sketchpad.pro
   * {@link http://sketchpad.pro|Sketchpad.pro - drawing board sketch free}
   *
  ...
  ToolAipen.prototype = Object.create(ToolFillable.prototype);
  
  Object.assign(ToolAipen.prototype, {
      color: {
          r: 0,
          g: 0,
          b: 0,
      ...
      smooth(ps) {
          "use strict";
          var i;
          var j, p0, p1, p, a = this.smoothFactor;
          //  ("ps", ps);
          if (ps.length < 2) {
              return;
          }
          var restCount = ps.length - this.smoothLength;
          if (restCount < 1) {
              restCount = 1;
          }
          for (i = restCount; i < ps.length; i += 1) {
              p0 = ps[i];
              p1 = ps[i - 1];
              p = {
                  x: p0.x * (1 - a) + p1.x * a,
      ...
      drawProjector(input) {
          // console.log(input);
          var config = {
              vpx: input.viewportX, //tofix: this same input fragment to send can have different viewport than this what was drawen
              vpy: input.viewportY,
              scl: input.scale,
              rot: input.rotation,
              wdh: input.width,
              hgt: input.height,
              // pts: pathData,
              uid: input.UID //overwitten by server;
          };
          // var toolConfig = input.getConfig();
          var sketchpad = this.sketchpad,
              context;
          context = null;
          // if (!toolConfig.lay || !toolConfig.lay.length) {
          ...
          var firstPoint = prePoints[0];
          context.beginPath();
          context.strokeStyle = input.color || this.getColorStr();
          context.lineCap = "miter";
          context.lineJoin = "miter";
          context.moveTo(firstPoint.x, firstPoint.y);
  
          //draw
          for (i = 1; i < prePoints.length; i += 1) {
  
              p1 = prePoints[i - 1];
              p2 = prePoints[i];
              var d = (this.distance(p1.x, p1.y, p2.x, p2.y)) / 10;
              if (d < 1) {
                  d = 1;
              }
              if (d > 2) {
      ...
      startInput: function startPath(id, x, y, e) {
          ...
          input.selectorDiv.style.display = "none";
          ...
          // input.addPoint(p.t, p.x, p.y);
          input.prePoints.push({t: now, x, y});
          this.drawProjector(input);
          return input;
      },
      moveInput: function moveInput(input, x, y, e) {
          ...
          input.lastX = x;
          input.lastY = y;
          input.prePoints.push({t: now, x: x, y: y});
          this.smooth(input.prePoints);
          // rest.forEach(function (p) {
          //     input.addPoint(p.t, p.x, p.y);
          // });
          var sx = input.prePoints[0].x;
          var sy = input.prePoints[0].y;
          var ex = x;
          var ey = y;
      ...
      finishInput: function finishInput(input, x, y, e) {
          ...
          this.drawProjector(input);
          this.updateBestFigure(input);
          input.prePoints.forEach(function (p) {
              input.addPoint(p.t, p.x, p.y);
          });
  
          sketchpad.clearProjector();
      },
      rotateInput(prePoints, angle) {
          var that = this;
          prePoints = prePoints.map(function (point) {
              return that.rotate(angle, point);
          });
          return prePoints;
      },
      getBoundingInputRect(prePoints) {
          // alert(1);
          var i;
          var input;
          const result = {
              ...
              width: 0,
              height: 0,
              centeredInput: JSON.parse(JSON.stringify(prePoints))
          };
          const pointsWage = {
              x: 0,
              y: 0,
          ...
          for (i = 0; i < prePoints.length; i += 1) {
              input = prePoints[i];
              pointsWage.x += input.x;
              pointsWage.y += input.y;
              pointsWage.t += input.t;
              if (result.maxX < input.x) {
                  result.maxX = input.x;
                  result.pointLeft = input;
              }
              if (result.maxY < input.y) {
                  result.maxY = input.y;
                  result.pointBottom = input;
              }
              if (result.minX > input.x) {
                  result.minX = input.x;
                  result.pointRight = input;
              }
              if (result.minY > input.y) {
                  result.minY = input.y;
                  result.pointTop = input;
              }
          }
          pointsWage.x /= prePoints.length;
          pointsWage.y /= prePoints.length;
          pointsWage.t /= prePoints.length;
          result.pointsWage = pointsWage;
          result.width = result.maxX - result.minX;
          result.height = result.maxY - result.minY;
          result.longestEdge = (result.width > result.height) ? result.width : result.height;
          result.avgEdge = (result.width * result.height);
          result.centerX = result.minX + (result.width / 2);
          result.centerY = result.minY + (result.height / 2);
          for (i = 0; i < result.centeredInput.length; i += 1) {
              input = result.centeredInput[i];
              input.x -= result.centerX
              input.y -= result.centerY
              input.distanceFromCenter = this.distance(0, 0, input.x, input.y);
          }
          result.area = result.width * result.height;
          result.cMinX =  result.centerX - (result.width / 2);
          result.cMaxX =  result.centerX + (result.width / 2);
          result.cMinY =  result.centerY - (result.height / 2);
          result.cMaxY =  result.centerY + (result.height / 2);
          result.firstPoint = result.centeredInput[0];
          result.lastPoint = result.centeredInput[result.centeredInput.length - 1];
          return result;
      },
      updateBestFigure(input) {
          const SHOW_HUDS = false;
          var i,
              rotation,
              point,
              detectors = [
           ...
           ;
          // back to normal
          // const prePoints = this.rotateInput(JSON.parse(JSON.stringify(input.prePoints)), -input.rotation);
          const prePoints = JSON.parse(JSON.stringify(input.prePoints));
          var that = this;
          var centeredInputRect = this.getBoundingInputRect(prePoints)
          let prevPoint;
          const lineDistance = centeredInputRect.centeredInput.reduce(function (previousValue, currentValue, index, array) {
               ;
              if (prevPoint) {
                  let result = previousValue + that.distance(prevPoint.x, prevPoint.y, currentValue.x, currentValue.y);
                  prevPoint = currentValue;
                  return result;
              }
              prevPoint = currentValue;
              return previousValue;
          }, 0);
          // console.log('%clineDistance', 'color:blue', lineDistance);
          var firstPoint = centeredInputRect.firstPoint;
          var lastPoint = centeredInputRect.lastPoint;
          var firstLastDistance = this.distance(firstPoint.x, firstPoint.y, lastPoint.x, lastPoint.y);
          var projections = {
              angle: [],
              maxWidth: 0,
              maxHeight: 0,
          ...
          for (rotation = 0; rotation < 360; rotation += 1) {
              var boundingInputRect = this.getBoundingInputRect(this.rotateInput(centeredInputRect.centeredInput, rotation));
              boundingInputRect.rotation = rotation;
              projections.angle[rotation] = boundingInputRect;
              if (projections.optimal === undefined || 
                  ((boundingInputRect.area <= projections.optimal.area))
                  // && (boundingInputRect.width >= projections.optimal.width)
                  // projections.optimal.area === boundingInputRect.area && projections.optimal.width < boundingInputRect.width || 
                  // projections.optimal.area === boundingInputRect.area && projections.optimal.width === boundingInputRect.width && 1
              ) {
                  if (projections.optimal && projections.optimal.area === boundingInputRect.area && projections.optimal.width === boundingInputRect.width && 1) {
                      console.log('bingo');
                  }
                  projections.minRectArea = boundingInputRect.area;
                  projections.optimal = boundingInputRect;
              }
              if (projections.horizontal === undefined || projections.maxWidth < boundingInputRect.width) {
                  projections.maxWidth = boundingInputRect.width;
                  projections.horizontal = boundingInputRect;
              }
              if (projections.vertical === undefined || projections.maxHeight < boundingInputRect.height) {
                  projections.maxHeight = boundingInputRect.height;
                  projections.vertical = boundingInputRect;
              }
          }
          projections.firstLastDistance = firstLastDistance;
          projections.isClosed = (firstLastDistance < Math.sqrt((projections.maxWidth * projections.maxWidth) * 0.05));
          console.log('%cPROJECTIONS', 'color:white;background:red', projections, centeredInputRect, projections.isClosed);
  
          var that = this;
          var detections = [];
          detectors.forEach(function (detector) {
              projections.angle.forEach(proj => {
                  detection = that[detector](projections, proj, input);
                  if (detection) {
                      detections.push(detection);
                  }    
              })
              // if (detection && (!bestDetection || bestDetection.missmatch > detection.missmatch)) {
              //     bestDetection = detection;
              // }
          });
          if (detections.length) {
              detections.sort(function (a, b) { return a.missmatch - b.missmatch});
              console.groupCollapsed(`Detections [${detections[0].detector}] ${detections[0].missmatch}`);
              detections.forEach(function (detection) {
                  console.log(`%c${detection.detector}`, 'background:darkblue;color:white', detection.missmatch);
              });
              console.groupEnd();
              var bestDetection = detections[0];
              if (bestDetection && bestDetection.missmatch < 0.05) {
                  setTimeout(function () {
                      if (!SHOW_HUDS) {
                          return;
                      }
                      that.drawProjector({
                          marker: true,
                          viewportX: centeredInputRect.viewportX,
                          viewportY: centeredInputRect.viewportY,
                          scale: centeredInputRect.scale,
                          rotation: -centeredInputRect.rotation,
                          width: centeredInputRect.width,
                          height: centeredInputRect.height,
                          size: 2,
                          color: 'gold',
                          prePoints: [
                              {x: centeredInputRect.cMinX, y: centeredInputRect.cMinY, t: 1},
                              {x: centeredInputRect.cMaxX, y: centeredInputRect.cMinY, t: 2},
                              {x: centeredInputRect.cMaxX, y: centeredInputRect.cMaxY, t: 3},
                              {x: centeredInputRect.cMinX, y: centeredInputRect.cMaxY, t: 4},
                              {x: centeredInputRect.cMinX, y: centeredInputRect.cMinY, t: 5},
                          ]
                      });
                      that.drawProjector({
                          marker: true,
                      ...
                      that.drawProjector({
                          marker: true,
                          viewportX: bestDetection.viewportX + centeredInputRect.viewportX,
                          viewportY: bestDetection.viewportY + centeredInputRect.viewportX,
                          scale: bestDetection.scale,
                          rotation: -bestDetection.rotation,
                          width: bestDetection.width,
                          height: bestDetection.height,
                          size: 6,
                          color: 'red',
                          prePoints: [
                              {x: centeredInputRect.pointsWage.x, y: centeredInputRect.pointsWage.y, t: 1},
                              {x: centeredInputRect.pointsWage.x, y: centeredInputRect.pointsWage.y + 1, t: 2},
                          ],
                      });
                      that.drawProjector({
                          marker: true,
                          ...
                          color: 'black',
                          prePoints: [
                              {x: centeredInputRect.centerX, y: centeredInputRect.centerY, t: 1},
                              {x: centeredInputRect.centerX, y: centeredInputRect.centerY + 1, t: 2},
                          ],
                      });
                      that.drawProjector({
                          marker: true,
                          ...
                          size: 2,
                          color: 'green',
                          prePoints: bestDetection.prePoints,
                      });
                      that.drawProjector({
                          marker: true,
                          viewportX: bestDetection.viewportX + centeredInputRect.viewportX,
                          ...
                          size: 2,
                          color: 'green',
                          prePoints: bestDetection.boundingInputRect.centeredInput,
                      });
                      that.drawProjector({
                          marker: true,
                          viewportX: bestDetection.viewportX + centeredInputRect.viewportX,
                          ...
                          color: 'red',
                          prePoints: [
                              {x: bestDetection.boundingInputRect.pointsWage.x, y: bestDetection.boundingInputRect.pointsWage.y, t: 1},
                              {x: bestDetection.boundingInputRect.pointsWage.x, y: bestDetection.boundingInputRect.pointsWage.y, t: 2},
                          ],
                      });
                      that.drawProjector({
                          marker: true,
                          ...
                          color: 'black',
                          prePoints: [
                              {x: bestDetection.boundingInputRect.centerX, y: bestDetection.boundingInputRect.centerY, t: 1},
                              {x: bestDetection.boundingInputRect.centerX, y: bestDetection.boundingInputRect.centerY, t: 2},
                          ],
                      });
                      that.drawProjector({
                          marker: true,
                          ...
                          color: 'ForestGreen',
                          prePoints: [
                              {x: bestDetection.boundingInputRect.cMinX, y: bestDetection.boundingInputRect.cMinY, t: 1},
                              {x: bestDetection.boundingInputRect.cMaxX, y: bestDetection.boundingInputRect.cMinY, t: 2},
                              {x: bestDetection.boundingInputRect.cMaxX, y: bestDetection.boundingInputRect.cMaxY, t: 3},
                              {x: bestDetection.boundingInputRect.cMinX, y: bestDetection.boundingInputRect.cMaxY, t: 4},
                              {x: bestDetection.boundingInputRect.cMinX, y: bestDetection.boundingInputRect.cMinY, t: 5},
                          ]
                      });
                      that.drawProjector({
                          marker: true,
                  ...
                  console.log('%cbest detection', 'background:darkgreen;color:white', bestDetection);
                  input.viewportX += (centeredInputRect.centerX + bestDetection.boundingInputRect.centerX) / input.scale;
                  input.viewportY += (centeredInputRect.centerY + bestDetection.boundingInputRect.centerY) / input.scale;
                  input.tool = bestDetection.tool;
                  input.rotation = bestDetection.boundingInputRect.rotation;
                  input.prePoints = bestDetection.prePoints;
              }
          }
      },
      // p - point
      ...
      getDistanceFromLine3(a, b, p) {
          var abd = this.distance(a.x, a.y, b.x, b.y);
          if (abd === 0) return this.distance(p.x, p.y, a.x, a.y);
          var t = ((p.x - a.x) * (b.x - a.x) + (p.y - a.y) * (b.y - a.y)) / abd;
          t = Math.max(0, Math.min(1, t));
          return this.distance(p.x, p.y, a.x + t * (b.x - a.x),a.y + t * (b.y - a.y));
      },
      getDistanceFromLine2(PA, PB, p) {
          var
              x1 = PA.x,
              y1 = PA.y,
              x2 = PB.x;
          ...
          // console.log(`y = m * x + b`);
          console.log(`y = (${m}) * x + (${b})`);
          // console.log(`y = (${m}) * x + (${b2})`);
  
          // A * x + B * y + C === 0
          var A = -m;
          if (A === 0 && B === 0) {
              console.error('A and B equals 0')
          }
          var C = -b;
          // console.log(`A * x + B * y + C = 0`);
          console.log(`(${A}) * x + (${B}) * y + (${C}) = 0`);
          var dpk = Math.abs(A * p.x + B * p.y + C) / Math.sqrt(A * A + B * B);
          console.log('get distance of ', p, ' from line between ', PA, PB, ' is ', dpk);
  
           ;
          return dpk;
          // find a, b
          // console.log('y = a * x + b', `y = ${a} * x + ${b}`);
          // console.log('calculate distance from point', p);
  
      },
  
      /* start: detectors */
      detectLine(projections, boundingInputRect) {
          // console.log('detectLine');
          // var boundingInputRect = projections.horizontal;
          var i;
          var input;
          let isLine = true;
          var missmatchSum = 0;
          if (projections.isClosed) {
              isLine = false;
          }
          for (i = 0; i < boundingInputRect.centeredInput.length; i += 1) {
              point = boundingInputRect.centeredInput[i];
              var missmatch = this.getDistanceFromLine({x: boundingInputRect.cMinX, y: 0}, {x: boundingInputRect.cMaxX, y: 0}, point);
              missmatchSum += missmatch;
          }
          const perimeter = boundingInputRect.width;
          const perimeterMissmatch = Math.abs(projections.lineDistance - perimeter) / perimeter;
          const distanceMissmatch = (missmatchSum / boundingInputRect.centeredInput.length) / boundingInputRect.avgEdge;
  
  
          // if (projections.lineDistance > boundingInputRect.width + boundingInputRect.width * this.MAX_PERCENT_ERROR) {
          //     isLine = false;
          // }
          // draw detected line
          if (isLine) {
              const autoInput = [];
              autoInput.push({
                  x: boundingInputRect.firstPoint.x,
                  y: boundingInputRect.firstPoint.y,
                  t: Date.now() + 0
              });
              autoInput.push({
                  x: boundingInputRect.lastPoint.x,
                  y: boundingInputRect.lastPoint.y,
                  t: Date.now() + 1
              });
              return {
                  boundingInputRect: boundingInputRect,
                  detector: "line",
                  perimeterMissmatch: perimeterMissmatch,
                  distanceMissmatch: distanceMissmatch,
                  missmatch: distanceMissmatch,
                  prePoints: autoInput,
                  tool: "line",
              };
          }
      ...
      detectTriangle(projections, boundingInputRect) {
          // console.log('detectTriangle');
          // var boundingInputRect = projections.horizontal;
          var i;
          var input;
          let isTriangle = true;
          ...
          const centeredInput = boundingInputRect.centeredInput;
          let missmatchSum = 0;
          var
              a = boundingInputRect.pointLeft,
              b = boundingInputRect.pointRight,
              c = boundingInputRect.pointTop;
  
          const perimeter = this.distance(a.x, a.y, b.x, b.y) + this.distance(b.x, b.y, c.x, c.y) + this.distance(c.x, c.y, a.x, a.y);
          if (!perimeter) {
              return;
          }
          for (i = 0; i < centeredInput.length; i += 1) {
              point = centeredInput[i];
              var bestDistance = Math.min(
                  this.getDistanceFromLine(a, b, point), // __
                  this.getDistanceFromLine(b, c, point), // 
                  this.getDistanceFromLine(c, a, point), // 
              );
              // console.log('triangle', i, bestDistance);
              missmatch = bestDistance;
              missmatchSum += missmatch;
          }
          const perimeterMissmatch = Math.abs(projections.lineDistance - perimeter) / perimeter;
          const distanceMissmatch = (missmatchSum / centeredInput.length) / boundingInputRect.avgEdge;
          ...
          if (isTriangle) {
              ...
              //     that.drawProjector({
              //         marker: true,
              //         viewportX: boundingInputRect.viewportX,
              //         viewportY: boundingInputRect.viewportY,
              //         scale: boundingInputRect.scale,
              //         rotation: -boundingInputRect.rotation,
              //         width: boundingInputRect.width,
              //         height: boundingInputRect.height,
              //         size: 2,
              //         color: 'green',
              //         prePoints: JSON.parse(JSON.stringify(autoInput)),
              //     });
              // }, 200);
              return {
                  boundingInputRect: boundingInputRect,
      ...
      detectRectangle(projections, boundingInputRect) {
          // console.log('detectRectangle');
          // var boundingInputRect = projections.optimal;
          var i;
          ...
          for (i = 0; i < centeredInput.length; i += 1) {
              point = centeredInput[i];
              var
                  a = {x: boundingInputRect.cMinX, y: boundingInputRect.cMinY},
                  b = {x: boundingInputRect.cMaxX, y: boundingInputRect.cMinY},
                  c = {x: boundingInputRect.cMaxX, y: boundingInputRect.cMaxY},
                  d = {x: boundingInputRect.cMinX, y: boundingInputRect.cMaxY};
              const distanceA = this.getDistanceFromLine(a, b, point);
              const distanceB = this.getDistanceFromLine(b, c, point);
              const distanceC = this.getDistanceFromLine(c, d, point);
              const distanceD = this.getDistanceFromLine(d, a, point);
              var bestDistance = Math.min(distanceA, distanceB, distanceC, distanceD);
              // console.log('rectangle', i, bestDistance);
              // console.log('point', point, 'a:', a, 'b:', b, 'c:', c, 'd:', d, 'da:', distanceA, 'db:', distanceB, 'dc:', distanceC, 'dd:', distanceD, 'best:', bestDistance);
              missmatch = bestDistance;
              missmatchSum += missmatch;
          }
  
          // projections.firstLastDistance;
          const perimeter = (boundingInputRect.width * 2 + boundingInputRect.height * 2);
          if (!perimeter) {
              return;
          }
          const perimeterMissmatch = Math.abs(projections.lineDistance - perimeter) / perimeter;
          const distanceMissmatch = (missmatchSum / centeredInput.length) / boundingInputRect.avgEdge;
          // console.log('rectangle avg', distanceMissmatch);
  
          // const perimeterMissmatchAvg = perimeterMissmatch / centeredInput.length;
          // const missmatchAvg = missmatchSum / centeredInput.length;
          const tolerance = 0.05;
          // if (perimeterMissmatch > tolerance) {
          //     isRectangle = false;
          // }
          if (distanceMissmatch > tolerance) {
              // isRectangle = false;
          }
  
          // if (missmatchAvg > tolerance) {
          //     isRectangle = false;
          // }
  
          // console.log(`%cisRectangle [%c${isRectangle}]`, 'background:black;color:white', 'background:black;color:white', 'P:', perimeter, 'L:', projections.lineDistance, 'E:', distanceMissmatch);
          if (isRectangle) {
              const autoInput = [];
              // autoInput.push({
              //     x: boundingInputRect.centerX - boundingInputRect.width / 2,
              //     y: boundingInputRect.centerY - boundingInputRect.height / 2,
              //     t: Date.now()
              // });
              // autoInput.push({
              //     x: boundingInputRect.centerX + boundingInputRect.width / 2,
              ...
              //     t: Date.now()
              // });
              autoInput.push({x: a.x, y: a.y, t: Date.now() - 4});
              autoInput.push({x: b.x, y: b.y, t: Date.now() - 3});
              autoInput.push({x: c.x, y: c.y, t: Date.now() - 2});
              autoInput.push({x: d.x, y: d.y, t: Date.now() - 1});
              autoInput.push({x: a.x, y: a.y, t: Date.now() - 0});
               ;
              return {
                  boundingInputRect: boundingInputRect,
                  detector: "rectangle",
      ...
      getEllipsePt(cx, cy, radiusX, radiusY, targetPtX, targetPtY) {
      ...
      detectEllipse(projections, boundingInputRect) {
          // return;
          // if (!projections.maxRange || Math.abs(projections.maxRange) / Math.abs(projections.maxHeight) < 0.2) {
          //     return;
          // }
          // var boundingInputRect = projections.horizontal;
          var i;
          ...
          let missmatchSum = 0;
          for (i = 0; i < centeredInput.length; i += 1) {
              input = centeredInput[i];
              closestPointOnEllipse = this.getEllipsePt(
                  0,
                  0,
                  boundingInputRect.width / 2,
                  boundingInputRect.height / 2,
                  input.x,
                  input.y
              );
              // console.log(closestPointOnEllipse, input);
              missmatch = this.distance(input.x, input.y, closestPointOnEllipse.x, closestPointOnEllipse.y);
              missmatchSum += missmatch;
          }
          const a = boundingInputRect.width / 2;
          const b = boundingInputRect.height / 2;
          const perimeter = this.ellipsePerimeter(a, b);
          if (!perimeter) {
              return;
          }
          const perimeterMissmatch = Math.abs(projections.lineDistance - perimeter) / perimeter;
          const distanceMissmatch = (missmatchSum / centeredInput.length) / boundingInputRect.avgEdge;
          // console.log('ellipse avg', distanceMissmatch);
          // const perimeterMissmatchAvg = perimeterMissmatch / centeredInput.length;
          // const missmatchAvg = missmatchSum / centeredInput.length;
          ...
          //     isEllipse = false;
          // }
          // if (missmatchAvg > Math.sqrt(boundingInputRect.width * boundingInputRect.height)) {
          //     isEllipse = false;
          // }
          // console.log(`%cisEllipse [%c${isEllipse}]`, 'background:black;color:white', 'background:black;color:white', 'P:', perimeter, 'L:', projections.lineDistance, 'E:', perimeterMissmatch);
  
          if (isEllipse) {
              // console.log("%cIS ELLIPSE", 'background:black;color:white', isEllipse);
              const autoInput = [];
              autoInput.push({
                  x: boundingInputRect.centerX,
                  y: boundingInputRect.centerY,
                  t: Date.now()
              });
              autoInput.push({
                  x: boundingInputRect.width / 2,
                  y: boundingInputRect.height / 2,
                  t: Date.now()
              });
              return {
                  boundingInputRect: boundingInputRect,
      ...
      detectCircle(projections, boundingInputRect) {
          // console.log('detectCircle');
          // var boundingInputRect = projections.optimal;
          // if (!boundingInputRect.centeredInputRect) {
          //     return;
          // }
          var i;
          var input;
          ...
          const centeredInput = boundingInputRect.centeredInput;
          let sumDistancesFromCenter = 0;
          centeredInput.forEach(function (input) {
              sumDistancesFromCenter += input.distanceFromCenter;
          });
          const avgDistancesFromCenter = sumDistancesFromCenter / centeredInput.length;
          let currentDistance;
          let missmatchSum = 0;
          for (i = 0; i < centeredInput.length; i += 1) {
              input = centeredInput[i];
              currentDistance = input.distanceFromCenter;
              ...
              missmatchSum += currentDistance;
              // console.log('missmatch', missmatch);
          }
          const missmatchAvg = missmatchSum / centeredInput.length;
          // console.log('detectCircle', missmatchAvg, avgDistancesFromCenter, Math.abs(missmatchAvg - avgDistancesFromCenter));
  
          const perimeter = 0;
          const tolerance = 0;
          const perimeterMissmatch = 0;
          const perimeterMissmatchAvg = 0;
          if (missmatchAvg > Math.sqrt(boundingInputRect.width * boundingInputRect.height)) {
              isCircle = false;
          }
          if (Math.abs(missmatchAvg - avgDistancesFromCenter)  > 0.1) {
              isCircle = false;
          }
          console.log(`%cisCircle [%c${isCircle}]`, 'background:black;color:white', 'background:black;color:white', 'P:', perimeter, 'L:', projections.lineDistance, 'E:', perimeterMissmatch);
  
          if (isCircle) {
              // console.log("%cIS CIRCLE", 'background:black;color:white', isCircle);
              const autoCircleInput = [];
              autoCircleInput.push({
                  x: boundingInputRect.centerX,
                  y: boundingInputRect.centerY,
                  t: Date.now()
              });
              autoCircleInput.push({
                  x: boundingInputRect.centerX + avgDistancesFromCenter,
                  y: boundingInputRect.centerY + avgDistancesFromCenter,
                  t: Date.now()
              });
              return {
                  boundingInputRect: boundingInputRect,
                  detector: "circle",
                  missmatch: missmatchSum / centeredInput.length,
                  fillColor: this.getFillColorStr(),
                  prePoints: autoCircleInput,
                  tool: "circle",
              };
          }
      },
  ...
  NSSketchpad.avaliableTools.push(ToolAipen);
  // @license magnet:?xt=urn:btih:0b31508aeb0634b347b8270c7bee4d411b5d4109&dn=agpl-3.0.txt AGPL-3 https://www.gnu.org/licenses/agpl-3.0-standalone.html
  /**
  ...
  Object.assign(ToolCirc.prototype, {
      ...
      drawFramePath: function drawFramePath(config, points) {
          ...
          for (ll = 0; ll < toolConfig.lay.length; ll += 1) {
              ...
              // console.log('draw-circle', points, toolConfig);
  ...
  // @license magnet:?xt=urn:btih:0b31508aeb0634b347b8270c7bee4d411b5d4109&dn=agpl-3.0.txt AGPL-3 https://www.gnu.org/licenses/agpl-3.0-standalone.html
  /**
   * @source: http://developers.sketchpad.pro/dist/src/sketchpad.tool.smoothpen.js
   *
   * Sketchpad.pro
   * {@link http://sketchpad.pro|Sketchpad.pro - drawing board sketch free}
   *
   ...
   * @param {object} config - tool config
   */
  function ToolSmoothpen(config) {
      "use strict";
      Tool.call(this, config);
  }
  
  
  ToolSmoothpen.prototype = Object.create(Tool.prototype);
  
  Object.assign(ToolSmoothpen.prototype, {
      color: {
          r: 0,
          g: 0,
          b: 0,
      ...
      moveInput: function moveInput(input, x, y, e) {
          ...
          // var p = {x: x, y: y};
              // clear.call(this);
          // if (this.distance(x, y, this.lastX, this.lastY) < this.minDist) {
          //     return;
          // }
          var now = new Date().getTime();
           ;
          // if (now - input.lastT < this.minTime) {
          //      ;
          //     return;
          // }
          input.lastT = now;
          ...
          input.lastY = y;
          input.preInput.push({t: now, x, y});
          this.smooth(input.preInput);
          // rest.forEach(function (p) {
          //     input.addPoint(p.t, p.x, p.y);
          // });
          var sx = input.preInput[0].x;
          var sy = input.preInput[0].y;
          var ex = x;
          var ey = y;
      ...
      finishInput: function finishInput(input, x, y, e) {
          ...
          this.drawProjector(input);
           ;
          input.preInput.forEach(function (p) {
              input.addPoint(now, p.x, p.y);
              // input.addPoint(p.t, p.x, p.y);
          });
          sketchpad.clearProjector();
      },
  ...
  NSSketchpad.avaliableTools.push(ToolSmoothpen);
  // @license magnet:?xt=urn:btih:0b31508aeb0634b347b8270c7bee4d411b5d4109&dn=agpl-3.0.txt AGPL-3 https://www.gnu.org/licenses/agpl-3.0-standalone.html
  /**
  ...
  function Sketchpad(config) {
      ...
      this.user = {};
      this.UID = config.user_id || name[0].toUpperCase() + name.substr(1);//Math.random() * 666;
      this.COLOR = config.user_color || randomColor();
      this.USER_NAME = config.user_name || this.UID;
      this.editorsCount = 0;
      this.viewersCount = 0;
  
  
      this.renderFadeEffectTransitionOut = {
          transition: "all 1s linear",
          opacity: 0.3
      }; // opacity
      this.renderFadeEffectTransitionIn = {
          transition: "all 1s linear",
          opacity: 1
      }; // opacity
      this.postponeChunkLength = 30;
      ...
      if (NSSketchpad.watermarkImageSrc) {
          this.watermark = new Image();//document.createElement("img");// create dom level element Image();
          ...
          this.watermark.addEventListener("click", function () {
              if (that.lastWatermarkSrc) {
                  that.watermark.src = that.lastWatermarkSrc;
                  that.lastWatermarkSrc = false;
                  return;
              }
              that.lastWatermarkSrc = that.watermark.src;
              var watermarkCanvas = document.createElement("canvas");
              watermarkCanvas.width = that.watermark.width;
              watermarkCanvas.height = that.watermark.height + that.watermark.width;
              var qrCode = new QRCode(new Image(), {
                  text: document.location.href,
                  width: that.watermark.width - 10,
                  height: that.watermark.width - 10
              });
              var qrCodeCanvas = qrCode[String('_' + 'oDrawing')][String('_' + 'elCanvas')];
              var watermarkCanvasContext = watermarkCanvas.getContext("2d");
              watermarkCanvasContext.drawImage(that.watermark, 0, that.watermark.width);
              watermarkCanvasContext.drawImage(qrCodeCanvas, 0, 0);
  
              that.watermark.src = watermarkCanvas.toDataURL("image/png");
          });
          this.watermark.src = NSSketchpad.watermarkImageSrc;
  
          // this.watermark.onload = function () {
          //     alert("watermark on load2");
          // };
      ...
      this.syncPointerDataFrequency = config.syncPointerDataFrequency || Math.round(1000 / 20);
      ...
      this.backgroundIFrame.id = "backgroundIFrame";
      ...
      this.canvasProjector.id = "canvasProjector";
      this.canvasProjector.style.opacity = 0.8;
      // this.canvasProjector.style.filter = "hue-rotate(90deg)";
      ...
      this.objectsCanva.id = "objectsCanva";
      ...
      this.crosshairEl.id = "crosshairEl";
  ...
  Object.assign(Sketchpad.prototype, {
      ...
      showGrid: function drawGrid(config) {
          ...
          function lineH(y, color) {
              line(-that.width / 2, y, that.width / 2, y, color);
          }
          function lineV(x, color) {
              line(x, - that.height / 2, x, that.height / 2, color);
          }
  
          lineH(0, "rgba(0,0,0,0.1)");
          lineV(0, "rgba(0,0,0,0.1)");
  
          var x, y;
          for (x = 0; x <= this.width / 2; x += step) {
              lineV(x, "rgba(255,0,0," + alpha + ")");
              lineV(-x, "rgba(255,0,0," + alpha + ")");
          }
          for (x = 0; x <= this.width / 2; x += substep) {
              lineV(x, "rgba(255,0,0," + subAlpha + ")");
              lineV(-x, "rgba(255,0,0," + subAlpha + ")");
          }
          for (y = 0; y <= this.height / 2; y += step) {
              lineH(y, "rgba(255,0,0," + alpha + ")");
              lineH(-y, "rgba(255,0,0," + alpha + ")");
          }
          for (y = 0; y <= this.height / 2; y += substep) {
      ...
      hideGrid: function hideGrid() {
          "use strict";
          this.displayGrid = false;
          this.contextProjector2D.clearRect(-this.width / 2, -this.height / 2, this.width, this.height);
      },
      clearProjector: function clearProjector() {
          this.contextProjector2D.clearRect(-this.width / 2, -this.height / 2, this.width, this.height);
      },
      clearSketchpad: function clearSketchpad() {
          ...
          if (this.centerViewport) {
              ...
              this.contextProjector2D.translate(this.width / 2, this.height / 2);
      ...
      refreshWindow: function refreshWindow(ignore) {//sourceTagInfo
          "use strict";
          Object.assign(this.canvas.style, this.renderFadeEffectTransitionOut);
          Object.assign(this.canvasB.style, this.renderFadeEffectTransitionOut);
           ...
           */
          if (!this.postponeDrawingVersion) {
              this.postponeDrawingVersion = 0;
          }
          this.postponeDrawingVersion = this.postponeDrawingVersion + 1;
          if (this.postponeDrawingVersion > 9999) {
              this.postponeDrawingVersion = 0; //reset counter
          }
          var postponeDrawingVersion = this.postponeDrawingVersion;
          var postponeSketch = this.room.sketch;
  
          var postponeChunkLength = this.postponeChunkLength;
          function postponeDrawing(commands, position) {
              // console.log('postponeDrawing', commands);
              ...
              if (i < commands.length) {
                  ...
                  Object.assign(that.canvas.style, that.renderFadeEffectTransitionIn);
                  Object.assign(that.canvasB.style, that.renderFadeEffectTransitionIn);
              }
          }
          if (this.room && (this.room.sketch instanceof Sketch)) {
              ...
              //     that.dispatch("changeViewport");
              //     that.dispatch("refreshWindow");
              // }, 0);//be nice for fade out animation
          }
      ...
      syncCanvasFrame: function syncCanvasFrame() {
          ...
          // screen size change
          var that = this;
          var RESIZE_REFRESH_BOUNCE_TIME = 0;
          if (
              ...
              this.planRefreshWindow(RESIZE_REFRESH_BOUNCE_TIME, "room.js:syncCanvasFrame:(resize)");
          }
          if (this.receivingHistory) {
      ...
      executeInputCommand: function executeInputCommand(obj) {
          ...
          case "nick-changed":
              ...
              if (obj.user_id && obj.user_name) {
                  ...
                  // console.log("nick-changed", obj);
                  this.room.updateClient(obj);
                  this.dispatch("updateClient", obj);
                  if (this.user && this.user.user_id && String(this.user.user_id) === obj.user_id) {
                      this.dispatch("my_nick_changed", obj);
                  }
                  // if (obj.user_id === )
                  // this.dispatch("user_data_change", obj);
              } else {
                  console.error("error nick-changed");
              }
              break;
          case "color-changed":
              if (obj.user_id && obj.user_color && obj.user_color_str) {
                  obj.user_id = String(obj.user_id);
                  // console.log("color-changed", obj);
                  this.room.updateClient(obj);
                  this.dispatch("updateClient", obj);
                  if (this.user && this.user.user_id && String(this.user.user_id) === obj.user_id) {
                      this.dispatch("my_color_changed", obj);
                  }
                  // if (obj.user_id === )
                  // this.dispatch("user_data_change", obj);
              } else {
                  console.error("error color-changed");
              }
              break;
