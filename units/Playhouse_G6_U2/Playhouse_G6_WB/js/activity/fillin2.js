...
FillIn.prototype = {
  ...
  validate: function () {
    ...
    for (var i = 0; i < elsQue.length; i++) {
      ...
      // =========================================
      // IGNORE QUESTIONS WITHOUT REAL ANSWERS
      // =========================================
      var hasRealAnswer =
        fDataObj.answer &&
        fDataObj.answer.length > 0 &&
        fDataObj.answer.some(function (ans) {
      ...
      if (!hasRealAnswer) {
        ...
        var emptyTick = elsQue[i].querySelector(".tick");
        var emptyCross = elsQue[i].querySelector(".cross");
        var emptyWrap = elsQue[i].querySelector(".icon_wrap");

        if (emptyTick) {
          emptyTick.style.display = "none";
        }

        if (emptyCross) {
          emptyCross.style.display = "none";
        }
    ...
    // =========================================
    // فقط الأسئلة اللي إلها جواب تدخل بالحسبة
    // =========================================

    var checkedResults = [];

    for (var r = 0; r < elsQue.length; r++) {
      var qIndex = parseInt(elsQue[r].dataset.qno);

      var qData = ob.data_obj.questions[qIndex - 1];

      var hasAnswer =
        qData.answer &&
        qData.answer.length > 0 &&
        qData.answer.some(function (ans) {
          return String(ans).trim() !== "";
        });

      if (hasAnswer) {
        checkedResults.push(resultArr[r]);
      }
    }

    allCorrect = checkedResults.length > 0 && checkedResults.indexOf(0) === -1;

    console.log(checkedResults, numOfFillIns);
