var snapshot_data = {
  snapshot: [
    {
      audio: [
        "../audios/P88/pizza.mp3",
        "../audios/P88/dough.mp3",
        "../audios/P88/sauce.mp3",
        "../audios/P88/sprinkle.mp3",
        "../audios/P88/cheese.mp3",
        "../audios/P88/add.mp3",
        "../audios/P88/pepperoni.mp3",
        "../audios/P88/slice.mp3",
      ],
      image: [
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word1.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word2.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word3.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word4.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word5.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word6.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word7.png",
        "../images/pages/snapshots/ARC_2_1_SB_U1_P5_word8.png",
      ],
      word: [
        "<span>pizza</span>",
        "<span>dough</span>",
        "<span>sauce</span>",
        "<span>sprinkle</span>",
        "<span>cheese</span>",
        "<span>add</span>",
        "<span>pepperoni</span>",
        "<span>slice</span>",
      ],
      imagePlacePos: [
        [1, 1, 1],
        [1, 1, 1],
        [1, 0, 1],
      ],
    },
  ],
  popuptitle: "What I Want To Know:",
};

var snapshotPopup_data =
  "<ul><li>What are the people doing?</li><li>Who are the pizzas for?</li><li>What toppings go on a pizza?</li></ul>";

var Popups_data = {
  slides: [
    {
      // 'apk': {
      //   "text": ["<div><p>I need your help.<br />Can you help me find the restaurant in the picture?</p></div>"],
      //   "audio": ["../audios/P88/pizza.mp3"]},
      // 'ccss': {
      //   "text": ["<span>1 Goodbye!</span>","<span>2 How are you?</span>","<span>3 Fine, thank you.</span>","<span>4 Hello!</span>","<span>5 Good morning!</span>"],
      //   "audio": ["../audios/P88/Pg4_Vocabulary_Adult Lady.mp3","../audios/P88/dough.mp3", "../audios/P88/pizza.mp3","../audios/P88/dough.mp3", "../audios/P88/pizza.mp3"]},
      listen: {
        backgroundImage: "../images/pages/page-1/1-word-background.png",
        mainTitle: "../images/pages/page-1/1-title.png",
        titleTexts: ["<span class='blue_text'>2</span> Listen again and read."],
        titleIcons: [""],
        titlesAudio: [
          "../audios/P88/Pg4_1.2_Adult_Lady.mp3",
          "../audios/P88/Pg4_1.3_Adult Lady.mp3",
          "../audios/P88/Pg4_1.4_Adult Lady.mp3",
        ],
        image: [
          "../images/pages/snapshots/page-1/popup-3-elem-1.png",
          "../images/pages/snapshots/page-1/popup-3-elem-2.png",
          "../images/pages/snapshots/page-1/popup-3-elem-3.png",
          "../images/pages/snapshots/page-1/popup-3-elem-1.png",
          "../images/pages/snapshots/page-1/popup-3-elem-2.png",
          "../images/pages/snapshots/page-1/popup-3-elem-3.png",
          "../images/pages/snapshots/page-1/popup-3-elem-1.png",
          "../images/pages/snapshots/page-1/popup-3-elem-2.png",
          "../images/pages/snapshots/page-1/popup-3-elem-3.png",
          "../images/pages/snapshots/page-1/popup-3-elem-3.png",
        ],
        words: [
          "<span class='number_bold'>1</span> wind power",
          "<span class='number_bold'>2</span> smog",
          "<span class='number_bold'>3</span> solar power",
          "<span class='number_bold'>4</span> carpool",
          "<span class='number_bold'>5</span> reuse",
          "<span class='number_bold'>6</span> compost",
          "<span class='number_bold'>7</span> conserve",
          "<span class='number_bold'>8</span> reduce",
          "<span class='number_bold'>9</span> sort",
          "<span class='number_bold'>10</span> disposable",
          "<span class='number_bold'>11</span> waste",
          "<span class='number_bold'>12</span> plastic bags",
        ],
        audio: [
          "../audios/P88/1_wind_power.mp3",
          "../audios/P88/2_smog.mp3",
          "../audios/P88/3_solar_power.mp3",
          "../audios/P88/4_carpool.mp3",
          "../audios/P88/5_reuse.mp3",
          "../audios/P88/6_compost.mp3",
          "../audios/P88/7_conserve.mp3",
          "../audios/P88/8_reduce.mp3",
          "../audios/P88/9_sort.mp3",
          "../audios/P88/10_disposable.mp3",
          "../audios/P88/11_waste.mp3",
          "../audios/P88/12_plastic_bags.mp3",
        ],
      },
      listen2: {
        // "leftImage": "../images/pages/snapshots/page-2/popup-2-left-img.png",
        mainTitle: "../images/pages/page-1/1-title.png",
        titleTexts: ["<span class='blue_text'>3</span> Ask and answer."],
        titleIcons: [""],
        titleTextRight: "<span class='blue_text'></span>",
        titlesAudio: ["../audios/new/p88ask.mp3"],
        image: [
          "../images/pages/page-1/2-img-1.png",
          "../images/pages/page-1/2-img-2.png",
        ],
        audio: ["../audios/new/p88/ask.mp3"],
      },
      listen3: {
        // "leftImage": "../images/pages/snapshots/page-2/popup-2-left-img.png",
        mainTitle: "../images/pages/page-1/1-title.png",
        mainTitle_audio: "../audios/P88/WORD_POWER.mp3",
        titleTexts: ["<span class='title-order'>1</span> Listen and say."],
        titleIcons: [""],
        titleTextRight:
          "<span class='blue_text'>Do you and your family recycle? What do you think is the most serious problem for the earth today? Does your school or neighbourhood do anything to help the environment?</span>",
        titlesAudio: ["../audios/P88/1_Listen_and_say.mp3"],
        image: [],
        audio: [
          "../audios/P88/Pg5_2.2_Adult Lady.mp3",
          "../audios/P88/Pg5_2.3_Adult Lady.mp3",
        ],
      },
    },
  ],
  class_name: ["", "", "", "", "", "", "", "", "", "", "", ""], //flex-row (or) flex-reverse (or) flex-column-reverse for each image-container **optional**
  words: [
    "wind power",
    "smog",
    "solar power",
    "carpool",
    "reuse",
    "compost",
    "conserve",
    "reduce",
    "sort",
    "disposable",
    "waste",
    "plastic bags",
  ],
  points: [
    "../images/pages/page-1/intro-1.png",
    "../images/pages/page-1/intro-2.png",
    "../images/pages/page-1/intro-3.png",
    "../images/pages/page-1/intro-4.png",
    "../images/pages/page-1/intro-5.png",
    "../images/pages/page-1/intro-6.png",
    "../images/pages/page-1/intro-7.png",
    "../images/pages/page-1/intro-8.png",
    "../images/pages/page-1/intro-9.png",
    "../images/pages/page-1/intro-10.png",
    "../images/pages/page-1/intro-11.png",
    "../images/pages/page-1/intro-12.png",
  ],
  popupDirections: [
    "bottom", //1
    "bottom", //2
    "bottom", //3
    "bottom", //4
    "bottom", //5
    "top", //6
    "bottom", //7
    "bottom", //8
    "top", //9
    "top", //10
    "bottom", //11
    "top", //12
  ],
  postions: [
    "left:33.5%; top:23.63%;",
    "left:41.75%; top:1.50%;",
    "left:51.50%; top:50.50%;",
    "left:30.88%; top:64.00%;",
    "left:21.00%; top:62.25%;",
    "left:22.75%; top:89.75%;",
    "left:38.75%; top:65.13%;",
    "left:59.88%; top:76.75%;",
    "left:38.50%; top:84.88%;",
    "left:53.63%; top:84.75%;",
    "left:31.50%; top:77.63%;",
    "left:58.50%; top:92.50%;",
  ],
  imagesAudio: [
    "../audios/P88/1_wind_power.mp3",
    "../audios/P88/2_smog.mp3",
    "../audios/P88/3_solar_power.mp3",
    "../audios/P88/4_carpool.mp3",
    "../audios/P88/5_reuse.mp3",
    "../audios/P88/6_compost.mp3",
    "../audios/P88/7_conserve.mp3",
    "../audios/P88/8_reduce.mp3",
    "../audios/P88/9_sort.mp3",
    "../audios/P88/10_disposable.mp3",
    "../audios/P88/11_waste.mp3",
    "../audios/P88/12_plastic_bags.mp3",
  ],
};
