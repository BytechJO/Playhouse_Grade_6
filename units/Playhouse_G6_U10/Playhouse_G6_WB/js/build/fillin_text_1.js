function buildFillInBody(aObj) {	
	...
	if(typeof aObj !=undefined && aObj !=null){		
		...
		if(aObj.image != 'no' && aObj.image != ""){
			...
			// htmlStmt += '<img class="link-imgl" src="../images/pages/activities/link.png"/>';
			// htmlStmt += '<img class="link-imgr" src="../images/pages/activities/link.png"/>';
			for (x = 0; x < numOfQuestions; x++) {
				...
				if (tmpObj.singleword) {
				  ...
				  qStr = words.map((word, index) => {
					...
					console.log("aObj.defaultAnswer", aObj.defaultAnswer , x+1);
					if (aObj.defaultAnswer == x+1) {
						inputboxstmt = '<input class="text_input_area" type="text" maxlength="' + tmpObj.maxlength + '" data-type="' + tmpObj.type + '" readonly value="' + ((aObj.questions)[currentQue - 1]).answer[0] + '">'
					} else {
						inputboxstmt = '<input class="text_input_area" type="text" maxlength="' + tmpObj.maxlength + '" data-type="' + tmpObj.type + '">'
					}
				  }).join(inputboxstmt);
				}
