function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    if (aObj.conversationPopup != undefined && aObj.conversationPopup != null) {
      htmlStmt += '<div class="conversation_popup_btn_wrap">';

      htmlStmt +=
        "<button " +
        'type="button" ' +
        'class="conversation_popup_btn" ' +
        'data-toggle="modal" ' +
        'data-target="#conversationReferenceModal">';

      htmlStmt += '<i class="fas fa-book-open"></i>';

      htmlStmt += "<span>" + aObj.conversationPopup.buttonText + "</span>";
    ...
    if (aObj.text != undefined && aObj.text != "") {
      htmlStmt +=
        '<div class="audioIcon off contant" data-audio="' +
        aObj.textAudio +
        '">';
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      if (tmpObj.options_words != undefined && tmpObj.options_words != "") {
        ...
        for (let y = 0; y < tmpObj.options_words.length; y++) {
          htmlStmt +=
            '<div class="audioIcon txt-audioIcon off d-flex contant " data-audio="' +
  ...
  if (aObj.image != undefined && aObj.image != "") {
    htmlStmt +=
      '<div class="image-container"><img src="' + aObj.image + '"/></div>';
  }
  // =====================================================================
  // CONVERSATION MODAL
  // =====================================================================
  ...
  if (aObj.conversationPopup != undefined && aObj.conversationPopup != null) {
    htmlStmt +=
      "<div " +
      'class="modal fade conversation_reference_modal" ' +
      'id="conversationReferenceModal" ' +
      'tabindex="-1" ' +
      'role="dialog" ' +
      'aria-hidden="true">';
    ...
    htmlStmt +=
      "<div " +
      'class="modal-dialog modal-dialog-centered modal-dialog-scrollable conversation_modal_dialog" ' +
      'role="document">';

    htmlStmt += '<div class="modal-content conversation_modal_content">';

    // =============================================
    // HEADER
    // =============================================

    htmlStmt += '<div class="modal-header conversation_modal_header">';

    htmlStmt += '<div class="conversation_modal_title">';

    htmlStmt += '<i class="fas fa-comments"></i>';

    htmlStmt += "<span>" + aObj.conversationPopup.title + "</span>";
    ...
    htmlStmt +=
      "<button " +
      'type="button" ' +
      'class="close conversation_close_btn" ' +
      'data-dismiss="modal" ' +
      'aria-label="Close">';

    htmlStmt += '<span aria-hidden="true">&times;</span>';
    ...
    htmlStmt += '<div class="modal-body conversation_modal_body">';

    for (var c = 0; c < aObj.conversationPopup.lines.length; c++) {
      var line = aObj.conversationPopup.lines[c];

      htmlStmt += '<div class="conversation_line">';

      htmlStmt +=
        '<div class="conversation_speaker">' + line.speaker + ":" + "</div>";

      htmlStmt += '<div class="conversation_text">' + line.text + "</div>";
    ...
    htmlStmt += '<div class="modal-footer conversation_modal_footer">';

    htmlStmt +=
      "<button " +
      'type="button" ' +
      'class="conversation_done_btn" ' +
      'data-dismiss="modal">' +
      "Back to Activity" +
      "</button>";
