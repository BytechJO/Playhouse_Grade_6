  ...
  listen: function (ob) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      inputs[i].addEventListener("input", function () {
        ...
        var currentQue = this.closest(".que");

        if (currentQue) {
          var questionInputs = currentQue.querySelectorAll("input");

          var currentIndex = Array.prototype.indexOf.call(questionInputs, this);

          /*
                  كل input ما عدا الأخير داخل السؤال
                  هو حرف واحد
                */
          var isLetterInput =
            currentIndex >= 0 && currentIndex < questionInputs.length - 1;

          if (isLetterInput) {
            // نخلي خانة الحرف حرف واحد فقط
            if (this.value.length > 1) {
              this.value = this.value.charAt(0);
            }

            // أول ما يكتب حرف ينتقل للي بعده
            if (
              this.value.length === 1 &&
              currentIndex < questionInputs.length - 1
            ) {
