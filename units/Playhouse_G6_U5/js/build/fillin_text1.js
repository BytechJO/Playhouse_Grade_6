function buildFillInBody(aObj) {
  ...
  if (typeof aObj != undefined && aObj != null) {
    ...
    if (aObj.options_img != undefined) {
      htmlStmt +=
        '<img src="' + aObj.options_img + '" class="options_img"></img>';
    }
    ...
    for (x = 0; x < numOfQuestions; x++) {
      ...
      if (tmpObj.singleword) {
        ...
        words.forEach(function (word, index) {
          if (index > 0) {
            // أول input = حرف واحد فقط
            var maxLength = inputIndex === 0 ? 1 : tmpObj.maxlength;
          ...
          if (word !== "") {
            ...
            qStr +=
              "<div " +
              'class="audioIcon txt-audioIcon off d-flex contant min_w_fit_contant" ' +
              'data-audio="' +
      ...
      htmlStmt += qStr;
      // if(tmpObj.Additional_text != undefined && tmpObj.Additional_text != ""){
      // 	for(let word=0; word<tmpObj.Additional_text.length; word++){
      // 		htmlStmt += '<div class="Additional_text">'+tmpObj.Additional_text[word]+'</div>';
      // 	}
      // }
