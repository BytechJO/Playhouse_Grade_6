...
function getCurrFileOrDirectory(aNeed, aPath) {
  var href =
    aPath == undefined && aPath == null ? document.location.href : aPath;
  var lastPathSegment = "";
  var currFolder = "";
  var unitFolder = "";
  if (href != null && href != undefined) {
    ...
    var pathArr = [];
    var path = href.substr(
      0,
      Math.max(href.lastIndexOf("/"), href.lastIndexOf("\\")),
    );
  ...
  if ($(this).hasClass("off")) {
    if (thisFile == "reading.html") {
      ...
      if (getReadingSlideNumber() == 1) {
        ...
        if (
          _readOneData.startTime != undefined &&
          _readOneData.endTime != undefined
        ) {
          if (_readOneData.startTime > 0 || _readOneData.endTime > 0) {
            headerAudioCtrl = new AudioControls(
              _templatePath + _readOneData.audio,
            );
            headerAudioCtrl.playAudio(
              $(this),
              _readOneData.startTime,
              _readOneData.endTime,
            );
          }
        } else {
          playThisAudio($(this));
  ...
  if (theAudio.src != "none") {
    ...
    theAudio.src =
      audioIndex == 0
        ...
        : _templatePath + audioArray[audioIndex];
...
function changeFullScreenIcon(aCondition) {
  if (aCondition == "exit") {
    $(".full_screen")
      .find("img")
      .attr("src", _templatePath + "./images/icons/exitfullscreen.png");
  } else {
    $(".full_screen")
      .find("img")
      .attr("src", _templatePath + "./images/icons/fullscreen.png");
  }
}
//--------------[ // Full screen functions ] ----------------------
...
function setLoadedStatus(val) {
  ...
  if (val == "templateMedia") {
    ...
    } else if (_currFile.startsWith("slide_text")) {
      ...
      if (_fileType != "" && _fileBuild != "") {
        if (_fileBuild == "yes") {
          switch (_fileType) {
            case "fillin":
              if (_fileSubType == "none") {
                ...
                if (_fileSubType == "order") {
                  if (
                    typeof fillin_order_data != undefined &&
                    fillin_order_data != null
                  ) {
            ...
            case "wordsearch":
              if (
                typeof wordsearch_data != undefined &&
                wordsearch_data != null
              ) {
            ...
            case "letterpath":
              if (
                typeof letterpath_data != undefined &&
                letterpath_data != null
              ) {
                buildLetterPathBody(letterpath_data);
              }
              break;
            case "prepcircleunderline":
              if (
                typeof prep_circle_underline_data != undefined &&
                prep_circle_underline_data != null
              ) {
                buildPrepCircleUnderlineBody(prep_circle_underline_data);
              }

              break;
            case "sentencecolour":
              if (
                typeof sentence_colour_data != undefined &&
                sentence_colour_data != null
              ) {
                buildSentenceColourBody(sentence_colour_data);
              }
...
function getIndexOfFile(aVal, aArr, aKey) {
  ...
  if (
    ...
    typeof aArr != undefined &&
    typeof aKey != undefined
  ) {
...
function doWindowResize() {
  ...
  if (!_alignInterval) {
    if ($cw.hasClass("activity_wrap")) {
      if (
        $(".activity_area").data("type") != undefined &&
        _loadingStatus == "loaded"
      ) {
...
function alignStage() {
  ...
  if (isMobile()) {
    ...
    if (
      $("header").find(".unitTitleText").prop("scrollWidth") >
      $("header").find(".unitTitleText").outerWidth() + 3
    ) {
      $("[rel='tooltip']").tooltip();
    } else {
